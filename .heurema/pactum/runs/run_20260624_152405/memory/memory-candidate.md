...
## Review Decisions
- f_001 [low] open internal/debate/config/config.go:61: The doc comment on config.Load is now stale: it still says Load 'then loads config, context, and personas', but context.md loading was removed by this change. The inline step-number comments inside Load were also only partially renumbered ('// 3. Parse personas' is followed by '// 5. Resolve panel' and '// 6. Resolve synthesizer'), so the sequence now skips 4.
- Proposal summary: pending=0 accepted=1 rejected=0

## Reusable Project Knowledge
- scope: in scope: Drop the Context field from config.Workspace and stop reading .heurema/debate/context.md in internal/debate/config.
- scope: in scope: Make internal/debate/runner assemble the debate brief from the task alone and remove every use of Workspace.Context in the runner and synthesizer.
- scope: in scope: Make debate init in cmd/debate scaffold only the two starter debater personas and no longer create a context.md file.
- scope: in scope: Update all affected tests so none assert context.md creation or a Workspace.Context field.
- scope: out of scope: Any change to grounding, the backends, personas, or the engine.
- scope: out of scope: Docs updates (DESIGN.md / SLICES.md) — handled separately.
- scope: out of scope: Introducing any new dependency or new feature.
