...
Current contract version: 9e29c0dbf6d7f0a7ce20928f301611bcf80ef7984777beddadcc6253b8b1d2be
...
**Assumptions**:
  ...
  - The opened ACP session cwd is the intended and trustworthy read root for this backend slice. This is a security boundary: the implementation must capture it once, canonicalize and symlink-evaluate it before use, and fail closed if that cwd cannot be trusted.
...
1. [codex-xhigh/assumptions-surfaced] The contract should explicitly state whether the ACP session cwd is expected to be a local filesystem path visible to the backend process. Root capture, absolute resolution, and symlink evaluation depend on host-local filesystem semantics; if adapters may expose remote/container paths, executor behavior becomes ambiguous.
   Evidence: Assumptions: "The opened ACP session cwd is the intended and trustworthy read root... canonicalize and symlink-evaluate it before use"; Acceptance criteria: "captures a canonical, absolute, symlink-evaluated read root from the opened ACP session cwd".
...
```json
{
  ...
  "base_version": "9e29c0dbf6d7f0a7ce20928f301611bcf80ef7984777beddadcc6253b8b1d2be",
  "contract": {
