...
- Approval: approved
- Contract hash: 455ab8357fa5696305bacdc0162ba70a1a68689aed7777ca3bda9b974d22c8d1

## Goal
Harden the ACP backend client tool contract so grounded file reads are explicitly advertised, root-scoped, and bounded, while non-read operations are denied conservatively and the existing timeout guard remains intact.
...
Items:
- mem_007 [fresh] score=66 — Implement an ACP backend transport under internal/backend/acp using github.co...
- mem_008 [fresh] score=57 — Implement an exec backend transport under internal/backend/exec (standard lib...
- mem_003 [fresh] score=52 — Slice 1: implement the policy-free engine on a mock backend. Package internal...
- mem_006 [fresh] score=49 — Slice 4: wire the cmd/debate CLI into a working debate on a deterministic off...
- mem_001 [stale] score=47 — Replace the hand-written stdlib flag parsing in cmd/debate with github.com/al...
  reason: changed file cmd/debate/e2e_test.go
  reason: changed file cmd/debate/scaffold.go
  reason: changed file docs/DESIGN.md
