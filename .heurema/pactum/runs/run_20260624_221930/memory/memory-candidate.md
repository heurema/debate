...
## Review Decisions
- f_001 [high] resolved internal/backend/acp/acp.go:278: Recovery can recapture a different read root for the same logical session when the original cwd path is a symlink.
  Resolution: Recovery now reuses the original clientImpl and its pinned read root instead of constructing a new client from the possibly moved symlink cwd; covered by TestSend_RecoveryPreservesCapturedReadRootForSymlinkCwd.
- f_002 [medium] resolved internal/backend/acp/acp.go:397: ReadTextFile has a validation-to-open race that can allow a symlink escape after the path check.
  Resolution: ReadTextFile now opens files through a pinned os.Root after root-relative resolution, eliminating the validation-to-open symlink escape race while preserving deterministic outside-root/path-escape errors.
- f_003 [high] resolved internal/backend/acp/acp.go:278: Recovery re-canonicalizes the original cwd instead of preserving the captured read root, so a symlink cwd can move the read boundary after a transparent session recovery.
  Resolution: Recovery preserves the captured canonical read root by keeping the same clientImpl across reopened ACP sessions; it no longer re-canonicalizes the original cwd during recovery.
- f_004 [medium] resolved internal/backend/acp/acp_test.go:682: Root-scoped read tests bypass the ACP session wiring, so the transport-level capture of the opened session cwd is not tested.
  Resolution: Added fake-agent ReadTextFile callback coverage through the ACP transport in TestOpen_ReadTextFileUsesSessionCwdRoot, including inside reads, direct outside-root denial, and symlink escape denial.
- Proposal summary: pending=0 accepted=4 rejected=0

## Reusable Project Knowledge
- scope: in scope: internal/backend/acp/acp.go: ACP InitializeRequest client capabilities, clientImpl construction/state, ReadTextFile, WriteTextFile, RequestPermission, and terminal-method error behavior.
- scope: in scope: internal/backend/acp/acp_test.go: focused fake ACP/client tests for advertised capabilities, root-scoped reads, line/limit bounded reads, path escape denial, permission option ordering, terminal denial, and preservation of timeout behavior.
- scope: in scope: internal/backend/acp/integration_test.go only if a compile-time adjustment is required by internal helper changes.
- scope: out of scope: Changing the public cmd/debate CLI, persona/config parsing, debate orchestration, engine loop, exec backend, or README/docs.
- scope: out of scope: Removing or weakening the existing DEBATE_ACP_OPEN_TIMEOUT and DEBATE_ACP_SEND_TIMEOUT safety boundary.
- scope: out of scope: Implementing a real terminal, write support, shell command execution, activity-based idle watchdog, or subprocess stderr ring-buffer in this slice.
- scope: out of scope: Adding network-dependent validation or requiring real ACP adapters in the default test suite.
- review_resolution: f_001 resolved: Recovery can recapture a different read root for the same logical session when the original cwd path is a symlink.; resolution: Recovery now reuses the original clientImpl and its pinned read root instead of constructing a new client from the possibly moved symlink cwd; covered by TestSend_RecoveryPreservesCapturedReadRootForSymlinkCwd.
- review_resolution: f_002 resolved: ReadTextFile has a validation-to-open race that can allow a symlink escape after the path check.; resolution: ReadTextFile now opens files through a pinned os.Root after root-relative resolution, eliminating the validation-to-open symlink escape race while preserving deterministic outside-root/path-escape errors.
- review_resolution: f_003 resolved: Recovery re-canonicalizes the original cwd instead of preserving the captured read root, so a symlink cwd can move the read boundary after a transparent session recovery.; resolution: Recovery preserves the captured canonical read root by keeping the same clientImpl across reopened ACP sessions; it no longer re-canonicalizes the original cwd during recovery.
- review_resolution: f_004 resolved: Root-scoped read tests bypass the ACP session wiring, so the transport-level capture of the opened session cwd is not tested.; resolution: Added fake-agent ReadTextFile callback coverage through the ACP transport in TestOpen_ReadTextFileUsesSessionCwdRoot, including inside reads, direct outside-root denial, and symlink escape denial.
...
- validation: go test -count=1 ./internal/backend/acp passed
- validation: go test -count=1 -run '^$' -tags acp_integration ./internal/backend/acp passed
...
- validation: bash -lc 'disallowed=$( { git diff --name-only -- .; git diff --cached --name-only -- .; git ls-files --others --exclude-standard -- .; } | sort -u | grep -Ev "^(internal/backend/acp/acp\.go|internal/backend/acp/acp_test\.go|internal/backend/acp/integration_test\.go|\.heurema/pactum/)" || true); test -z "$disallowed"' passed
