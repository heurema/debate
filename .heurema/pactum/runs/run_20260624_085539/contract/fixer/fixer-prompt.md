...
Current contract version: c6fc6d6b1b918cf711b9b5884ecc938df5ce9940fdd149bc35c1c491f1917e7f
...
**Acceptance criteria**:
  ...
  - signal.Parse returns (zero Signal, false) when there is no `signal` block, the block is not valid JSON, or required structure is missing; non-`signal` fenced blocks are ignored and surrounding prose — both before and after the last signal block — does not affect parsing.
...
**Validation commands**:
  ...
  - ! bash scripts/dep-guard.sh ./internal/debate/... github.com/heurema/debate/internal/debate
...
1. [codex-xhigh/completeness] The contract does not fully specify what counts as a valid signal object when fields are missing or have zero values.
   Evidence: Acceptance says Parse unmarshals JSON object `{position, objections, done}` and returns false when `required structure is missing`, but does not explicitly say whether all three fields must be present, whether `objections` may be null, or whether empty `position` is valid.
2. [codex-xhigh/validation-soundness] The negative validation command relies on shell negation, which may not be executable by a validation gate that runs commands directly rather than through a shell.
   Evidence: Validation commands include: `! bash scripts/dep-guard.sh ./internal/debate/... github.com/heurema/debate/internal/debate`.
3. [codex-xhigh/assumptions-surfaced] The contract leaves the first-round Progress baseline implicit.
   Evidence: “Assess sets RoundResult.Progress ... to true iff the set of open objections this round differs from the previous round's; the verdict tracks the previous round's open-objection set across calls.”
...
```json
{
  ...
  "base_version": "c6fc6d6b1b918cf711b9b5884ecc938df5ce9940fdd149bc35c1c491f1917e7f",
  "contract": {
