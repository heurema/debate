...
Current contract version: ef9922e083fd6936b59dba3162eb602e652036d95f7effd0cf09a5d52a287801
...
**Acceptance criteria**:
  ...
  - transport exposes Transport.Open(ctx, Spec) (Session, error), Session.Send(ctx, prompt string) (Result, error), Session.Close() error, Result{Content string, Usage Usage}, a Usage struct of token counters, and a Spec carrying id, model, effort, system, read-only, and optional command. Classify(err error) ErrorClass{Retryable bool, Kind string} maps nil to a non-retryable `none` class and classifies known categories deterministically (rate_limit/idle_timeout/transport_drop/server_error/deadline -> retryable; auth/client_error/canceled -> non-retryable; unrecognized -> non-retryable `unknown`).
...
1. [codex-xhigh/completeness] The transport error classification API does not define the concrete error values or wrappers that Classify must recognize.
   Evidence: Acceptance criteria say: `Classify(err error) ErrorClass{Retryable bool, Kind string} maps nil to ... and classifies known categories deterministically (rate_limit/idle_timeout/transport_drop/server_error/deadline -> retryable; auth/client_error/canceled -> non-retryable; unrecognized -> non-retryable unknown).`
2. [codex-xhigh/testability] The error-classification acceptance criterion does not define machine-checkable input errors for the named classes.
   Evidence: “Classify(err error) ErrorClass{Retryable bool, Kind string} maps nil to a non-retryable `none` class and classifies known categories deterministically (rate_limit/idle_timeout/transport_drop/server_error/deadline -> retryable; auth/client_error/canceled -> non-retryable; unrecognized -> non-retryable `unknown`).”
3. [codex-xhigh/assumptions-surfaced] The contract does not explicitly state what orchestrate.Run should return in Result.Transcript when a PromptBuilder or Session.Send error occurs after some turns have already been appended in the current round.
   Evidence: Acceptance criteria say: “If PromptBuilder or Session.Send returns an error during a round, orchestrate.Run surfaces it to loop.Run as the Step error…” and loop error paths define completed-round Outcome semantics, but there is no matching assumption or criterion for whether partially accumulated transcript turns from the failed round are returned.
...
```json
{
  ...
  "base_version": "ef9922e083fd6936b59dba3162eb602e652036d95f7effd0cf09a5d52a287801",
  "contract": {
